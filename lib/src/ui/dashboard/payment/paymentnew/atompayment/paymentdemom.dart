...
Future<bool> validateSignature(
  ...
  if (data['payInstrument']['payDetails']['signature'] == genSig) {
    ...
    print("signature does not matched");
    return false;
  }
}
...
class _PayPageState extends State<PayPage> {
  ...
  Future<void> initiatePayment() async {
    ...
    try {
      ...
      //Json data for sending to atom server - Modified with custom return URL
      String jsonData =
                            ...
                            MaterialPageRoute(
                                builder: (context) => PaymentWebView(
                                      ...
                                      userPhone: widget
                                          ...
                                          .data!
                                          .mobileNO
                                          .toString(), // Fixed: was using email instead of mobileNO
...
class _PaymentWebViewState extends State<PaymentWebView>
  ...
  bool _hasReturnedFromUPI = false;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    // When app comes back to foreground after UPI payment
    if (state == AppLifecycleState.resumed &&
        _isPaymentProcessing &&
        !_hasReturnedFromUPI) {
      _hasReturnedFromUPI = true;
      debugPrint("App resumed from UPI payment - checking transaction status");
  ...
  // Method to check transaction status when returning from UPI
  Future<void> _checkTransactionStatus() async {
    try {
      debugPrint("Checking transaction status for: ${widget.currentTxnId}");

      // Option 1: Reload the webview to check for updates
      if (mounted) {
        await webViewController.reload();
      }

      // Option 2: Call your API to check transaction status
      // You can implement this based on your backend API
      // await _checkTransactionStatusFromAPI();
    } catch (e) {
      debugPrint("Error checking transaction status: $e");
    }
  }

  // Optional: API call to check transaction status
  Future<void> _checkTransactionStatusFromAPI() async {
    try {
      // Implement your API call to check transaction status
      // This is a placeholder - replace with your actual API endpoint
      final response = await http.get(
        Uri.parse(
            '${ServerHelper.baseUrl}payment/status/${widget.currentTxnId}'),
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("Transaction status from API: $statusData");

        if (statusData['status'] == 'success') {
          _handlePaymentSuccess(statusData);
        } else if (statusData['status'] == 'failed') {
          _handlePaymentFailure();
        }
      }
    } catch (e) {
      debugPrint("Error checking transaction status from API: $e");
    }
  }
  ...
  void _handlePaymentSuccess(Map<String, dynamic>? responseData) {
    if (!mounted) return;

    BlocProvider.of<MainBloc>(context)
        .add(GetCapturePaymentResponse(atomId: widget.atomTokenId.toString()));
  }

  void _handlePaymentFailure() {
    if (!mounted) return;

    _closeWebView(context, "Transaction Failed");
          ...
          builder: (context, state) {
            return Stack(
              children: [
                  ...
                  onLoadStop: (controller, url) async {
                    ...
                    if (url != null &&
                      ...
                      try {
                        ...
                        await _processPaymentResponse(response);
                      } catch (e) {
                ...
                if (state is GettingPreparepaymentcapture)
                  Container(
  ...
  Future<void> _processPaymentResponse(String response) async {
    try {
      ...
      if (response.trim().contains("cancelTransaction")) {
        ...
        if (jsonInput != null) {
          _handlePaymentSuccess(jsonInput);
        }
      }
    } catch (e) {
      debugPrint("Error processing payment response: $e");
  ...
  void _closeWebView(BuildContext context, String transactionResult) {
    debugPrint("Closing web view with result: $transactionResult");
    if (!mounted) return;
...
//   //   try {
//   //     final url = Uri.parse(
//   //         'https://nirmalaapi.lumensof.in/api/Paymet/api/payment/capture-response');
