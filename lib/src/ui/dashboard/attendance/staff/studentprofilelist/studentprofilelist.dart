...
class StudentProfileList extends StatefulWidget {
  const StudentProfileList({super.key});

  @override
  State<StudentProfileList> createState() => _StudentProfileListState();
}
    ...
    return Scaffold(
      ...
      body: BlocConsumer<MainBloc, MainState>(
        listenWhen: (previous, current) =>
            current is ClassStaffListSuccess ||
            current is ClassStaffSectionListSuccess,
        listener: (context, state) {
          if (state is ClassStaffListSuccess) {
            BlocProvider.of<MainBloc>(context)
                .add(GettingStaffClassSectionList(classId: state.classId));
