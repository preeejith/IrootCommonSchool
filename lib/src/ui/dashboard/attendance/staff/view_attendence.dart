...
class ViewAttendanceScreen extends StatelessWidget {
                ...
                children: [
                  ...
                  Obx(() {
                    ...
                    return customDropDown55(
                      ...
                      logic.staffHomeWorkController.staffClassListModel?.data ??
                          [],
                      (selectedValue) async {
                  ...
                  Obx(() {
                    return customDropDown55(
                        ...
                        if (logic.sectionList.isNotEmpty) {
                          try {
                            final sectionData = logic.sectionList.firstWhere(
                                (item) => item['sectionName'] == selectedValue);
                            logic.selectedSectionId.value =
                      ...
                      isClass: false,
                      isLoading: logic.isSectionLoading.value,
                    );
                  }),
                  const SizedBox(height: 10),
  ...
  // Interactive dropdown widget (same as in StaffAttendanceScreen)
  ...
  DataRow buildDataRow(int index, ViewAttendanceController logic) {
    ...
    if (index >= logic.studentListData.length) {
      return _createEmptyDataRow(index, logic.allBetweenDates.length);
    }

    var student = logic.studentListData[index];

    // Handle null student case
    if (student == null || student.attendance.isEmpty) {
      return _createEmptyDataRow(index, logic.allBetweenDates.length);
    }

    List<DataCell> cells = [
      datacell("${index + 1}"),
      datacell(student.attendance[0]?.className ?? "N/A"),
      datacell(student.attendance[0]?.studentName ?? "N/A"),
      datacell(student.attendance[0]?.sectionName ?? "N/A"),
      for (var date in logic.allBetweenDates)
        allbuildDateComparisonWidget(date, student),
      datacell(student.attendancePer ?? "N/A"),
    ];

    return DataRow(cells: cells);
  }

// Alternative approach - return DataRow with placeholder values
  DataRow buildDataRowWithPlaceholder(
      int index, ViewAttendanceController logic) {
    // Check if index is within bounds
    if (index >= logic.studentListData.length) {
      return _createEmptyDataRow(index, logic.allBetweenDates.length);
    }
    ...
    // Handle null student case with placeholder values
    if (student == null || student.attendance.isEmpty) {
      return _createEmptyDataRow(index, logic.allBetweenDates.length);
    }
...
// Helper method to create empty DataRow
  DataRow _createEmptyDataRow(int index, int dateCount) {
    List<DataCell> cells = [
      datacell("${index + 1}"),
      datacell("N/A"), // className
      datacell("N/A"), // studentName
      datacell("N/A"), // sectionName
      for (int i = 0; i < dateCount; i++) datacell("N/A"), // attendance dates
      datacell("N/A"), // attendancePer
    ];
