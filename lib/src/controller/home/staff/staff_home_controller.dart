...
//   String dataItemName = "";
//   String? dataItemId = "";
//   bool? isclassteacherany = false;
//   Rx<StaffDetail> staffDetail = StaffDetail().obs;

//   // Staff? staffClass;
//   StaffClassModel? staffClassmodel;
//   StaffClassListModel? staffClassListModel;
//   StaffSectionListModel? staffSectionListModel;
...
//             // _getstaffSectionNew(loginResponse.data![0].staffid.toString(),),
//           }
...
//     }
//   }
//   _getstaffSectionNew(String? staffId,String? sectionId) async {
//     // staffClassmodel

//     final String url =
//         '${ServerHelper.baseUrl}Exam/GetClassList?staff_Id=$staffId&classId=$sectionId';
//     try {
...
//       if (response.statusCode == 200) {
//         return StaffSectionListModel.fromJson(jsonDecode(response.body));
//       } else {
//         print('Failed to load data: ${response.statusCode}');
//         return null;
//       }
...
//     }
//   }
//   _getstaffclassNew(String? staffId) async {
//     // staffClassmodel
...
// Section Model for new API response
class SectionData {
  final int? sectionId;
...
class StaffSectionListResponse {
  ...
  factory StaffSectionListResponse.fromJson(Map<String, dynamic> json) {
    return StaffSectionListResponse(
      data: json['data'] != null
          ? (json['data'] as List)
              .map((item) => SectionData.fromJson(item))
              .toList()
          : null,
      msg: json['msg'],
...
//fetching the dropdown data
          staffClassmodel =
              await _getstaffclass(loginResponse.data![0].staffid.toString());
          staffClassListModel = await _getstaffclassNew(
              loginResponse.data![0].staffid.toString());
          print("My Class Model");
          print(staffClassListModel);
          ...
          if (staffClassmodel != null) {
            ...
            //   }
            // }
            for (int i = 0; i < staffClassListModel!.data!.length; i++) {
              if (staffClassListModel!.data![i].isClassTeacher == true) {
                isclassteacherany = true;
                dataItemName =
                    staffClassListModel!.data![i].dataListItemName.toString();
                dataItemId =
                    staffClassListModel!.data![i].dataListItemId.toString();
              }
            }
            // _getstaffSectionNew(loginResponse.data![0].staffid.toString(),),
          }

          if (staffClassListModel != null) {
            staffSectionModel = await _getstaffSection(
                loginResponse.data![0].staffid.toString(), dataItemId);
            print("dnhj");
            if (staffClassListModel != null) {
  ...
  // NEW METHOD: Get sections based on staff ID and class ID using the new API
  Future<StaffSectionListResponse?> getStaffSectionList(
      String staffId, String classId) async {
    final String url =
        '${ServerHelper.baseUrl}Exam/GetStaffSectionList?staffId=$staffId&classId=$classId';

    log('Fetching sections from: $url');
    ...
    try {
      ...
      log('Section API Response Status: ${response.statusCode}');
      log('Section API Response Body: ${response.body}');

      if (response.statusCode == 200) {
        var decoded = jsonDecode(response.body);
        return StaffSectionListResponse.fromJson(decoded);
      } else {
        print('Failed to load section data: ${response.statusCode}');
        return null;
      }
    } catch (e) {
...
//   }
  ...
  _getstaffclassNew(String? staffId) async {
    ...
    final String url =
        '${ServerHelper.baseUrl}Exam/GetStaffClassList?staff_Id=$staffId';
    try {
      final response = await http.post(
        Uri.parse(url),
        headers: {'Content-Type': 'application/json'},
      ...
      if (response.statusCode == 200) {
        return StaffClassListModel.fromJson(jsonDecode(response.body));
      } else {
        print('Failed to load data: ${response.statusCode}');
        return null;
      }
