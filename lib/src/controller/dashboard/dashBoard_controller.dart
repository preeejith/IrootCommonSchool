...
import '../../modal/attendance/new/adminstaffmodel.dart';
...
class DashBoardController extends GetxController {
  ...
  AdminStaffListModel? adminStaffListModel = AdminStaffListModel();
  ...
  void onInit() {
    ...
    _getAdminCourse();
    _getAdminStaff();
    userRole == "Administrator" ? _getAdminStaffList() : "";
          ...
          for (var attenClass in classResponse.data) {
            adminCourseDataList.add(StaffData(
                ...
                staffId: attenClass.subjectId == ""
                    ? null
                    : int.parse(attenClass.subjectId.toString())));
          }
        }
      }
      update();
...
//new route for staff list
  Future<void> _getAdminStaffList() async {
    try {
      // AdminStaffListModel adminStaffListModel = AdminStaffListModel();
      final response = await http.get(
          Uri.parse("${baseUrlName}Exam/GetStaffList"),
          headers: credentials);

      if (response.statusCode == 200) {
        adminStaffListModel =
            AdminStaffListModel.fromJson(await ServerHelper.get(
